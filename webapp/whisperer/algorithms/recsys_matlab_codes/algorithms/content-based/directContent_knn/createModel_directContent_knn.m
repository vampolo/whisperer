    ...
    if (exist('provaCosShrink')==3)
        ...
        if (~isfield(param,'noNormalization'))
            if param.shrinkNormalization>0 
                ...
                icm=normalizeShrinkageColsMatrix(icm,param.shrinkNormalization);
            else
                display('matrix normalized by using cosine');
                icm=normalizeColsMatrix(icm);
            end
        else
            display('icm will not be normalized');
        end
    end
...
display('Cosine IIknn: (MEMORY OPTIMIZATION) multiplication and knn-filtering will be comptuted together');     
    ...
    II = sparse(size(icm,2),size(icm,2));     % creo matrice vuota (sparsa)
    ...
    for j=1:splitNum
        ...
        if ~cfailed        
            try
                [colItems,commonElementsC] = provaCosShrink (icm,icm(:,colIndexes));
                commonEl=true;
            catch e
                %display(e);
        ...
        if (SimilarityShrinkage)
            if (~commonEl)
                commonElements=zeros(size(icm,2),length(colIndexes));
                otherSplitSize=100;
                otherSplitNum=ceil(size(icm,2)/otherSplitSize);
                for a=1:otherSplitNum
                    otherColMax=min([a*otherSplitSize,size(icm,2)]);
                    otherColIndexes=otherSplitSize*(a-1)+1:otherColMax;
                    for b=1:length(colIndexes)
                        commonElements(otherColIndexes,b)=sum(spones(icm(:,otherColIndexes).*(icm(:,colIndexes(b))*ones(1,length(otherColIndexes)))),1);
                    end
     ...
     if isfield(param,'computeBaseline')
         if islogical(param.computeBaseline)
             if param.computeBaseline
                 [mu,bu,bi] = computeRatingBiases (icm,lambdaU,lambdaI);
                 model.mu = mu;
                 model.bu = bu;
                 model.bi = bi;
             end
