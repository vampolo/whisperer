function  [A,rows,cols,entries,rep,field,symm] = mmread(filename, offset)
%
...
%      and symm (symmetry).
%
%      offset is optional (default=0). It indicates the starting indexes in the
%      matrixMarker file. offset=0 states that indexes start from 0.
...
if ( strcmp(rep,'coordinate')) %  read matrix given in sparse 
  ...
  if  ( strcmp(field,'real') )               % real valued entries:
    ...
    A = sparse(T(:,1)+offset, T(:,2)+offset, T(:,3), rows , cols);
