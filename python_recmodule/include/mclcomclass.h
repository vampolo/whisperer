...
public:
	...
	virtual HRESULT UpdateRegistry(BOOL bRegister)
    {
      ...
      if (bRegister)
      {
        ...
        try
        {
        wDllPath = new OLECHAR[strlen(szDllPath) + 1];
        mbstowcs(wDllPath, szDllPath, strlen(szDllPath) + 1); // +1 means don't forget the terminating null character
    ...
    virtual bool InitializeComponentInstanceEx(HMCRINSTANCE* inst)
    {
        ...
        bool bResult = m_pInitializeEx(inst, getPath(), ctfStream, (int)ctfSize);
...
public:
    ...
    bool InitializeComponentInstanceEx(HMCRINSTANCE* inst)
    {
        ...
        if (!m_mcrInstance)
        {
            ...
            mclCtfStream ctfStream = mclGetStreamFromArraySrc(ctfData, ctfSize);
            ret = m_pInitializeEx(&m_mcrInstance, getPath(), ctfStream, (int)ctfSize);
